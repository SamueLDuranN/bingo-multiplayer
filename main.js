...
const generateCard = () => {
    const numbers = Array.from({ length: 75 }, (_, i) => i + 1); // Números del 1 al 75
    const shuffled = numbers.sort(() => Math.random() - 0.5); // Mezclar los números
    return shuffled.slice(0, 25); // Tomar los primeros 25 números para el cartón
};
...
io.on("connection", (socket) => {
    ...
    socket.on("join-room", (username, roomId) => {
        ...
        roomData.cards[socket.id] = generateCard(); // Generar un cartón único
        console.log(`Cartón para ${username}: ${roomData.cards[socket.id]}`);
    });
